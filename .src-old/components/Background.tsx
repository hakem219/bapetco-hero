    ...
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float gradientFactor = smoothstep(-50.0, 100.0, vWorldPosition.y);
          
          // Base gradient from dark bottom to slightly lighter top
          vec3 color = mix(uColorBottom, uColorTop, gradientFactor);
          
          // Add subtle radial glow at horizon
          float horizonGlow = 1.0 - abs(gradientFactor - 0.3);
      ...
      `,
      uniforms: {
        uColorTop: { value: new THREE.Color('#0D1525') },
        uColorBottom: { value: new THREE.Color('#060A12') },
...
// Ambient floating particles in far background
export function AmbientStars() {
  const geometry = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      positions[i * 3 + 1] = radius * Math.sin(phi) * Math.sin(theta) * 0.5 + 20; // Bias upward
