...
void main() {
  ...
  pos.x += snoise(position * aNoiseFreq + vec3(noiseTime, 0.0, 0.0)) * 3.0;
  pos.y += snoise(position * aNoiseFreq + vec3(0.0, noiseTime, 100.0)) * 2.0;
  pos.z += snoise(position * aNoiseFreq + vec3(0.0, 0.0, noiseTime + 200.0)) * 3.0;
  ...
  float pulse = 1.0 + 0.2 * sin(uTime * 1.5 + aDelay * 10.0);
  gl_PointSize = aSize * distanceAttenuation * pulse * 2.0;
  gl_PointSize = clamp(gl_PointSize, 0.5, 8.0);
  
  // Fade out as build progresses
  vAlpha = uFogDensity * (1.0 - uBuildProgress * 0.8);
  vDepth = -mvPosition.z;
}
`;
...
void main() {
  ...
  // Shimmer effect
  float shimmer = 0.7 + 0.3 * sin(uTime * 2.0 + vDepth * 0.1);
  
  // Depth fade
  float depthFade = smoothstep(150.0, 30.0, vDepth);
  
  vec3 color = uColor * shimmer;
  
  gl_FragColor = vec4(color, alpha * vAlpha * depthFade * 0.4);
}
`;
