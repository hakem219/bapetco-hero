...
// Camera keyframes - UPDATED to position rig on FAR RIGHT of screen
// Camera looks from the left side, rig appears on right
const cameraKeyframes = [
  { progress: 0.0, position: [25, 45, 100], target: [25, 25, 0] },
  { progress: 0.15, position: [0, 40, 90], target: [25, 22, 0] },
  { progress: 0.3, position: [-25, 35, 75], target: [25, 20, 0] },
  { progress: 0.5, position: [-45, 28, 60], target: [25, 18, 0] },
  { progress: 0.7, position: [-55, 22, 55], target: [25, 16, 0] },
  { progress: 0.85, position: [-60, 20, 58], target: [25, 15, 0] },
  { progress: 1.0, position: [-65, 22, 65], target: [-25, 18, 0] }, // Final: rig fully visible on FAR RIGHT
];
...
export function CameraController({ progress }: CameraControllerProps) {
  ...
  const targetRef = useRef(new THREE.Vector3(25, 25, 0));
  const positionRef = useRef(new THREE.Vector3(25, 45, 100));
  ...
  useFrame((state) => {
    ...
    if (progress > 0.98) {
      const breathe = Math.sin(state.clock.elapsedTime * 0.3) * 0.12;
      const breathe2 = Math.cos(state.clock.elapsedTime * 0.2) * 0.08;
      camera.position.x += breathe;
      camera.position.y += breathe2 * 0.25;
    }
  });
